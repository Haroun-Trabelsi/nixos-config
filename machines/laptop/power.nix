...
lib.mkMerge [
  {
    # Both specialisations of this portable base (laptop and desktop) keep
    # root and disk swap on the same removable SSD, so routine paging never
    # touches that link either way: zram takes priority 100 by default,
    # ahead of the disk swap partition declared in hardware-shared.nix, which
    # stays as a low-priority backstop for whatever zram can't hold.
    zramSwap = {
      enable = true;
      algorithm = "zstd";
      memoryPercent = 50;
